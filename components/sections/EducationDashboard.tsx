...
export default function EducationDashboard() {
  ...
  const { data: news, isLoading: newsLoading } = useQuery({
    queryKey: ["news", "education"],
    queryFn: () => getNews("education"),
  })

  const { data: events, isLoading: eventsLoading } = useQuery({
    queryKey: ["events", "education"],
    queryFn: () => getEvents("education"),
  })

  const { data: services, isLoading: servicesLoading } = useQuery({
    queryKey: ["services", "education"],
    queryFn: () => getServices("education"),
  })

  const { data: facilities, isLoading: facilitiesLoading } = useQuery({
    queryKey: ["facilities", "education"],
    queryFn: () => getFacilities("education"),
  })
  ...
  const { mutate: deleteRecord, isPending: isDeleting } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [activeTab, "education"] })
      setDeleteItem(null)
    },
  })
  ...
  const { mutate: saveNews, isPending: isSavingNews } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["news", "education"] })
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <NewsFormModal
        ...
        onSubmit={(data) => saveNews({ ...data, sector: "education" })}
        editItem={editNews}
